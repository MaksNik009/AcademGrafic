/* ═══════════════════════════════════════════════════════════════════════════
   АКАДЕМГРАФИК — app.js v5 (полный, исправленный)
   Добавлено:
     - фиолетовый цвет дежурных, красный для blackout
     - независимость корпусов (поле building в duties/lessons)
     - система шаблонов (сохранение/загрузка/выбор активного шаблона)
   Все существующие функции сохранены, только дополнены.
   ═══════════════════════════════════════════════════════════════════════════ */
...
function normEntry(entry) {
  ...
  return { tid: entry.tid || entry.id || '', dept: entry.dept || null, building: entry.building || '1' };
}
    ...
    week.forEach(({ d, key, dow }) => {
      ...
      if (isHoliday) {
        ...
        contentHtml = dutyEntries.map(entry => {
          ...
          const replaceBadge = (isReplace && isMyDutyHere) ? '<div style="font-size:.68rem;color:var(--orange);margin-top:3px">🔄 Просит замену</div>' : '';
          const removeBtn = State.currentRole === 'admin' ? `<button class="acc-remove-duty mob-only" data-remove-tid="${entry.tid}" data-remove-dept="${entry.dept||''}" data-remove-key="${key}" title="Убрать дежурного">✕</button>` : '';
          return `<div class="acc-duty-chip${isReplace && isMyDutyHere ? ' replace' : ''}" style="border-left-color:${color};background:${color}12;border-color:${color}40;margin-bottom:4px;position:relative">${removeBtn}<div class="acc-duty-name">${teacher.name}</div><div class="acc-duty-dept">${deptLabel}</div>${teacher.phone ? `<div class="acc-duty-dept">${teacher.phone}</div>` : ''}${replaceBadge}</div>`;
        }).join('');
      } else {
...
function openModal(key, day, month, year, mode = 'assign') {
  ...
  if (assignedEntries.length > 0) {
    ...
    assignedEntries.forEach(entry => {
      ...
      html += `<div class="modal-assigned-row"><div class="opt-avatar" style="background:${color};width:28px;height:28px;font-size:.7rem;cursor:pointer" onclick="openTeacherInfoModal('${entry.tid}')" title="О преподавателе">${initials(t.name)}</div><div style="flex:1;min-width:0"><span class="modal-assigned-name">${t.name.split(' ').slice(0,2).join(' ')}</span>${deptLabel ? `<span style="display:block;font-size:.7rem;color:var(--text-muted);font-family:var(--font-mono)">${deptLabel}</span>` : ''}</div><button class="modal-remove-one" data-tid="${entry.tid}" data-dept="${entry.dept||''}" title="Снять">✕</button></div>`;
    });
    html += `</div><div class="modal-section-label">Добавить ещё преподавателя:</div>`;
  }
  if (State.teachers.length === 0) {
    html += `<div class="empty-state" style="padding:1rem 0"><div class="empty-icon">👤</div><p class="empty-title">Нет преподавателей</p><p class="empty-sub">Перейдите во вкладку «Преподаватели»</p></div>`;
  } else {
    ...
    State.teachers.forEach((t, idx) => {
      ...
      html += `<button class="teacher-option${selected ? ' selected' : ''}" data-id="${t.id}" role="option" aria-selected="${selected}"><div class="opt-avatar" style="background:${color}" onclick="event.stopPropagation();openTeacherInfoModal('${t.id}')" title="Инфо о преподавателе" role="button" tabindex="0">${initials(t.name)}</div><div class="opt-info"><div class="opt-name">${t.name}</div><div class="opt-meta">нед: ${wc}/${t.maxLoad}${t.phone ? ' · ' + t.phone : ''}</div>${deptSelector}</div>${badge}</button>`;
    });
    html += `</div>`;
  }
  ...
  body.querySelectorAll('.teacher-option:not([disabled])').forEach(btn => {
    btn.addEventListener('click', () => {
      body.querySelectorAll('.teacher-option').forEach(b => { b.classList.remove('selected'); b.setAttribute('aria-selected', 'false'); });
...
function renderDayPanel(key) {
  ...
  if (dutyEntries.length > 0) {
    dutyStrip.innerHTML = `<div style="font-size:.72rem;color:var(--text-muted);font-family:var(--font-mono);margin-bottom:6px;text-transform:uppercase;letter-spacing:.05em">Дежурные</div><div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:4px">` + dutyEntries.map(e => {
      const t = teacherById(e.tid);
      if (!t) return '';
      const color = getColor(teacherIndex(e.tid));
      return `<div style="display:flex;align-items:center;gap:5px;background:${color}14;border:1px solid ${color}40;border-radius:20px;padding:3px 10px 3px 4px;cursor:pointer" onclick="openTeacherInfoModal('${e.tid}')"><div style="width:22px;height:22px;border-radius:50%;background:${color};display:flex;align-items:center;justify-content:center;font-size:.58rem;font-weight:700;color:#fff;flex-shrink:0">${initials(t.name)}</div><span style="font-size:.75rem;font-weight:500;color:var(--navy)">${t.name.split(' ').slice(0,2).join(' ')}</span>${isAdmin ? `<button onclick="event.stopPropagation();removeDutyFromPanel('${key}','${e.tid}','${e.dept||''}')" style="background:none;border:none;color:var(--text-faint);cursor:pointer;font-size:.8rem;padding:0 0 0 2px" title="Убрать">✕</button>` : ''}</div>`;
    }).join('') + `</div>` + (isAdmin ? `<button class="day-panel-add-btn" onclick="openModal('${key}',${dd},${mm-1},${y},'assign')">+ Добавить дежурного</button>` : '');
  } else {
    dutyStrip.innerHTML = isHoliday ? `<div class="day-panel-holiday">🏛 ${getHolidayName(key)}</div>` : `<div style="font-size:.82rem;color:var(--text-faint);font-style:italic">Дежурных не назначено</div>${isAdmin ? `<button class="day-panel-add-btn" onclick="openModal('${key}',${dd},${mm-1},${y},'assign')">+ Назначить дежурного</button>` : ''}`;
  }
  ...
  pairsEl.innerHTML = PAIRS.map(p => {
    ...
    const entriesHtml = validEntries.length ? validEntries.map((e, i) => {
      const t = teacherById(e.tid);
      const color = getColor(teacherIndex(e.tid));
      return `<div class="pair-teacher-row"><div style="width:28px;height:28px;border-radius:50%;background:${color};display:flex;align-items:center;justify-content:center;font-size:.65rem;font-weight:700;color:#fff;flex-shrink:0;cursor:pointer" onclick="openTeacherInfoModal('${e.tid}')">${initials(t.name)}</div><div style="flex:1;min-width:0"><div style="font-size:.82rem;font-weight:600;color:var(--navy)">${t.name.split(' ').slice(0,2).join(' ')}</div><div style="font-size:.7rem;color:var(--text-muted);font-family:var(--font-mono)">${e.dept || t.dept || ''}${e.room ? ' · 🚪 ' + e.room : ''}</div></div>${isAdmin ? `<button class="pair-remove-btn" onclick="removePairEntry('${key}',${p.n},${i})" title="Удалить">✕</button>` : ''}</div>`;
    }).join('') : `<div style="font-size:.75rem;color:var(--text-faint);font-style:italic;padding:4px 0">— свободно —</div>`;
    return `<details class="pair-block" ${validEntries.length ? 'open' : ''}><summary class="pair-summary"><span class="pair-num">Пара ${p.n}</span><span class="pair-time">${p.time}</span><span class="pair-count">${validEntries.length > 0 ? validEntries.length + ' преп.' : ''}</span><span class="pair-chevron">▾</span></summary><div class="pair-body"><div class="pair-teachers-list" id="pair-list-${key}-${p.n}">${entriesHtml}</div>${isAdmin ? `<div class="pair-add-row" id="pair-add-${key}-${p.n}"><div class="select-wrap" style="flex:1;min-width:0"><select class="field-input field-select pair-teacher-sel" style="height:32px;font-size:.78rem" id="pair-tsel-${key}-${p.n}" onchange="updatePairDeptSel('${key}',${p.n})"><option value="">— Преподаватель —</option>${State.teachers.map(t => `<option value="${t.id}">${t.name.split(' ').slice(0,2).join(' ')}</option>`).join('')}</select></div><div id="pair-dept-wrap-${key}-${p.n}" style="display:none;min-width:0;flex:1"><select class="field-input field-select" style="height:32px;font-size:.75rem" id="pair-dept-${key}-${p.n}"></select></div><input class="field-input" placeholder="Кабинет" style="width:72px;height:32px;font-size:.78rem" id="pair-room-${key}-${p.n}"/><button class="day-panel-add-btn" style="padding:0 10px;height:32px;font-size:.75rem" onclick="addPairEntry('${key}',${p.n})">+</button></div>` : ''}</div></details>`;
  }).join('');
}
...
function _buildWelcomePage(page) {
  ...
  if (page === 'login') return `<button class="wm-back" id="wmBack" title="Назад">←</button><div style="text-align:center;margin-bottom:1.25rem"><div style="font-size:2.2rem;margin-bottom:.4rem">🔐</div><h2 class="wm-title" style="margin-bottom:.25rem">Вход для Завуча</h2><p class="wm-hint">Введите логин и пароль</p></div><div style="display:flex;flex-direction:column;gap:.75rem"><input class="field-input" type="text" id="authLogin" placeholder="Логин" autocomplete="username" style="font-size:1rem"/><input class="field-input" type="password" id="authPassword" placeholder="Пароль" autocomplete="current-password" style="font-size:1rem"/><div style="display:flex;align-items:center;gap:.75rem;margin-top:.25rem"><button class="btn-modal-save" id="authSubmit" style="flex:1;padding:.7rem">Войти</button><span id="authStatus" style="font-size:1.4rem;width:28px;text-align:center;flex-shrink:0"></span></div></div>`;
  if (page === 'picker') {
    const buildRows = (filter = '') => State.teachers.map(t => {
      ...
      if (norm) { const idx = nameLower.indexOf(norm); displayName = t.name.slice(0, idx) + `<mark style="background:#FFF176;border-radius:2px;padding:0 1px">${t.name.slice(idx, idx + norm.length)}</mark>` + t.name.slice(idx + norm.length); }
      return `<button class="wm-teacher-card" data-tid="${t.id}"><div class="wm-teacher-av" style="background:${color};width:44px;height:44px;font-size:.85rem">${initials(t.name)}</div><div class="wm-teacher-card-name">${displayName}</div><div class="wm-teacher-dept" style="font-size:.65rem">${t.dept || ''}</div></button>`;
    }).join('');
    return `<button class="wm-back" id="wmBack" title="Назад">←</button><h2 class="wm-title" style="margin-bottom:.5rem">Выберите преподавателя</h2><input class="field-input" id="wmTeacherSearch" placeholder="🔍 Поиск по имени или фамилии…" style="margin-bottom:.75rem;font-size:.9rem" autocomplete="off"/><div class="wm-teacher-grid" id="wmTeacherGrid">${buildRows()}</div>`;
  }
  if (page === 'teacher-login') {
    const tid = State._pendingTeacherId;
    const t = teacherById(tid);
    const color = t ? getColor(teacherIndex(tid)) : '#4A90D9';
    return `<button class="wm-back" id="wmBack" title="Назад">←</button><div style="text-align:center;margin-bottom:1.25rem">${t ? `<div style="width:60px;height:60px;border-radius:50%;background:${color};display:flex;align-items:center;justify-content:center;font-size:1.2rem;font-weight:700;color:#fff;margin:0 auto .75rem">${initials(t.name)}</div>` : ''}<h2 class="wm-title" style="margin-bottom:.25rem">${t ? t.name : 'Преподаватель'}</h2><p class="wm-hint">Введите пароль для входа</p></div><div style="display:flex;flex-direction:column;gap:.75rem"><input class="field-input" type="password" id="tAuthPassword" placeholder="Пароль" autocomplete="current-password" style="font-size:1rem"/><div style="display:flex;align-items:center;gap:.75rem"><button class="btn-modal-save" id="tAuthSubmit" style="flex:1;padding:.7rem">Войти</button><span id="tAuthStatus" style="font-size:1.4rem;width:28px;text-align:center;flex-shrink:0"></span></div></div>`;
  }
  return '';
}
function _wireWelcomePage(page) {
  if (page === 'choose') {
    ...
    document.getElementById('wmChooseTeacher')?.addEventListener('click', async () => {
      if (State.teachers.length === 0 && sb) { const btn = document.getElementById('wmChooseTeacher'); if (btn) btn.style.opacity = '0.6'; for (let i = 0; i < 30; i++) { await new Promise(r => setTimeout(r, 100)); if (State.teachers.length > 0) break; } if (btn) btn.style.opacity = ''; }
  ...
  if (page === 'login') {
    ...
    const doLogin = () => {
      ...
      if (login === ADMIN_LOGIN && pass === ADMIN_PASSWORD) { if (status) status.textContent = '✅'; setTimeout(() => { hideWelcomeModal(); applyRole('admin'); }, 350); }
      else { if (status) status.textContent = '❌'; const pwd = document.getElementById('authPassword'); if (pwd) pwd.value = ''; setTimeout(() => { const s = document.getElementById('authStatus'); if (s) s.textContent = ''; }, 1500); }
    };
  ...
  if (page === 'picker') {
    ...
    wireCards();
    search?.addEventListener('input', () => { grid.innerHTML = buildRows(search.value.trim()); wireCards(); });
  ...
  if (page === 'teacher-login') {
    ...
    const doTeacherLogin = () => {
      ...
      if (pass === ADMIN_PASSWORD) { if (status) status.textContent = '✅'; setTimeout(() => { State.currentTeacherId = State._pendingTeacherId; delete State._pendingTeacherId; hideWelcomeModal(); applyRole('teacher'); }, 350); }
      else { if (status) status.textContent = '❌'; const pwd = document.getElementById('tAuthPassword'); if (pwd) pwd.value = ''; setTimeout(() => { const s = document.getElementById('tAuthStatus'); if (s) s.textContent = ''; }, 1500); }
    };
...
function renderDeptManager(selected) {
  ...
  if (!container) return;
  const selectedList = selected.length ? selected.map((d, i) => `<div style="display:flex;align-items:center;gap:6px;margin-bottom:4px"><input class="field-input" style="flex:1;height:32px;font-size:.8rem" value="${d.replace(/"/g,'&quot;')}" data-dept-i="${i}"/><button type="button" class="modal-remove-one" data-dept-del="${i}" title="Удалить кафедру">✕</button></div>`).join('') : '<div style="font-size:.78rem;color:var(--text-faint);padding:4px 0">Нет кафедр</div>';
  container.innerHTML = `<label class="field-label">Кафедры <span class="required">*</span></label><div id="tDeptsList">${selectedList}</div><div style="display:flex;gap:6px;margin-top:6px;align-items:center;flex-wrap:wrap"><div class="select-wrap" style="flex:1;min-width:120px"><select class="field-input field-select" id="tDeptPickSel" style="height:34px;font-size:.8rem"><option value="">— выбрать из списка —</option>${globalDepts.map(d => `<option value="${d.replace(/"/g,'&quot;')}">${d}</option>`).join('')}</select><svg class="select-arrow" viewBox="0 0 16 16" fill="none" width="12" height="12"><path d="M4 6l4 4 4-4" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></svg></div><button type="button" class="btn btn--outline" id="tDeptAddFromList" style="padding:.3rem .7rem;font-size:.78rem;height:34px">+ Добавить</button></div><div style="display:flex;gap:6px;margin-top:4px;align-items:center"><input class="field-input" id="tDeptNewInput" placeholder="Новая кафедра…" style="flex:1;height:32px;font-size:.8rem"/><button type="button" class="btn btn--outline" id="tDeptAddNew" style="padding:.3rem .7rem;font-size:.78rem;height:34px;white-space:nowrap">+ Создать</button></div>`;
  container.querySelectorAll('[data-dept-i]').forEach(input => { input.addEventListener('change', () => { const i = parseInt(input.dataset.deptI); _modalDepts[i] = input.value.trim(); }); });
  container.querySelectorAll('[data-dept-del]').forEach(btn => { btn.addEventListener('click', () => { const i = parseInt(btn.dataset.deptDel); _modalDepts.splice(i, 1); renderDeptManager(_modalDepts); }); });
...
function openTeacherInfoModal(tid) {
  ...
  document.getElementById('teacherInfoBody').innerHTML = `<div style="display:flex;align-items:center;gap:14px;margin-bottom:1.25rem"><div style="width:56px;height:56px;border-radius:50%;background:${color};display:flex;align-items:center;justify-content:center;font-size:1.2rem;font-weight:700;color:#fff;flex-shrink:0">${initials(t.name)}</div><div><div style="font-size:1.05rem;font-weight:700;color:var(--navy)">${t.name}</div>${t.phone ? `<div style="font-size:.82rem;color:var(--text-muted);font-family:var(--font-mono);margin-top:3px">📞 ${t.phone}</div>` : ''}</div></div><div style="margin-bottom:.75rem"><div class="modal-section-label" style="margin-bottom:6px">Кафедры</div><div>${deptsHtml || '<span style="color:var(--text-faint);font-size:.82rem">—</span>'}</div></div><div style="display:flex;gap:1rem;margin-bottom:.75rem"><div style="flex:1;background:var(--surface-2);border-radius:var(--radius-sm);padding:10px;text-align:center"><div style="font-size:1.5rem;font-weight:700;color:${color}">${dutyCount}</div><div style="font-size:.7rem;color:var(--text-muted);font-family:var(--font-mono)">дежурств в месяце</div></div><div style="flex:1;background:var(--surface-2);border-radius:var(--radius-sm);padding:10px;text-align:center"><div style="font-size:1.5rem;font-weight:700;color:${color}">${t.maxLoad}</div><div style="font-size:.7rem;color:var(--text-muted);font-family:var(--font-mono)">макс. дн/нед</div></div></div>${blackouts.length ? `<div><div class="modal-section-label" style="margin-bottom:6px">🚫 Нежелательные даты</div><div style="font-size:.8rem;color:var(--text-secondary);font-family:var(--font-mono)">${blackouts.join(', ')}</div></div>` : ''}<div class="modal-actions" style="margin-top:1.25rem"><button class="btn-modal-clear" onclick="closeModal('teacherInfoOverlay')">Закрыть</button>${State.currentRole === 'admin' ? `<button class="btn-modal-save" onclick="closeModal('teacherInfoOverlay');openTeacherModal('${tid}')">✏️ Редактировать</button>` : ''}</div>`;
...
function renderModalBlackouts(dates, tid) {
  ...
  const tagsHtml = sorted.length ? sorted.map(k => { const d = new Date(k + 'T00:00:00'); return `<span class="blackout-tag" style="font-size:.75rem;padding:3px 8px">${d.getDate()} ${MONTHS_RU_GEN[d.getMonth()]} ${d.getFullYear()}<button class="blackout-remove" data-k="${k}" style="margin-left:4px;background:none;border:none;cursor:pointer;color:inherit">✕</button></span>`; }).join('') : '<span style="font-size:.78rem;color:var(--text-faint)">Нет нежелательных дат</span>';
  container.innerHTML = `<div class="field-group field-group--wide" style="margin-top:.75rem"><label class="field-label">🚫 Нежелательные даты (алгоритм пропустит их)</label><div style="display:flex;gap:.5rem;align-items:center;margin-bottom:.5rem"><input type="date" class="field-input" id="tBlackoutInput" style="flex:1;min-width:0"/><button type="button" class="btn btn--outline" id="tBlackoutAddBtn" style="white-space:nowrap;padding:.45rem .85rem;font-size:.8rem">+ Добавить</button></div><div id="tBlackoutTags" style="display:flex;flex-wrap:wrap;gap:.35rem;min-height:1.5rem">${tagsHtml}</div></div>`;
  document.getElementById('tBlackoutAddBtn').addEventListener('click', () => { const input = document.getElementById('tBlackoutInput'); const val = input.value; if (!val) { showToast('Выберите дату', 'error'); return; } if (!_modalBlackouts) _modalBlackouts = []; if (_modalBlackouts.includes(val)) { showToast('Уже добавлено', 'info'); return; } _modalBlackouts.push(val); input.value = ''; renderModalBlackouts(_modalBlackouts, tid); });
...
function saveTeacherModal() {
  ...
  if (editId) {
    const t = teacherById(editId);
    if (t) { Object.assign(t, { name, dept, depts, phone, maxLoad, building }); State.blackoutDates[editId] = [...(_modalBlackouts || [])]; t.blackoutDates = State.blackoutDates[editId]; }
...
function renderTeachersList() {
  ...
  container.innerHTML = ['1','2','3'].map(b => `<details class="building-section" open><summary class="building-section-header"><span class="building-section-label">🏢 ${b} корпус</span><span class="building-section-count">${byBuilding[b].length} преп.</span><span class="building-section-chevron">▾</span></summary><div class="building-section-table">${byBuilding[b].length === 0 ? `<div class="building-empty">Нет преподавателей</div>` : `<table class="teacher-table"><thead><tr><th>ФИО</th><th>Кафедра</th><th>Телефон</th><th style="text-align:center">Нагр.</th><th style="text-align:center">Дн.</th><th></th></tr></thead><tbody>${byBuilding[b].map(({ t, idx }) => { const color = getColor(idx); const dc = getMonthDutyCount(t.id); return `<tr><td><div style="display:flex;align-items:center;gap:8px"><div style="width:28px;height:28px;border-radius:50%;background:${color};display:flex;align-items:center;justify-content:center;font-size:.62rem;font-weight:700;color:#fff;flex-shrink:0">${initials(t.name)}</div><span style="font-size:.82rem;font-weight:600;color:var(--navy)">${t.name}</span></div></td><td style="font-size:.75rem;color:var(--text-muted)">${t.dept}</td><td style="font-size:.75rem;font-family:var(--font-mono)">${t.phone || '—'}</td><td style="text-align:center;font-size:.78rem">${t.maxLoad}</td><td style="text-align:center;font-size:.78rem;font-weight:600;color:${dc > 0 ? 'var(--blue)' : 'var(--text-faint)'}">${dc}</td><td><div style="display:flex;gap:4px;justify-content:flex-end"><button class="t-remove" style="padding:4px 8px;border:1px solid var(--border);border-radius:var(--radius-sm);font-size:.7rem;background:var(--surface);cursor:pointer" data-edit="${t.id}">✏️</button><button class="t-remove" data-id="${t.id}"><svg viewBox="0 0 16 16" fill="none" width="13" height="13"><path d="M3 4h10M6 4V2.5h4V4M5.5 4l.5 9M10.5 4l-.5 9" stroke="currentColor" stroke-width="1.4" stroke-linecap="round"/></svg></button></div></td></tr>`; }).join('')}</tbody></table>`}</div></details>`).join('');
...
function renderStats() {
  ...
  grid.innerHTML = State.teachers.map((t, idx) => {
    ...
    return `<div class="stat-card"><div class="stat-header"><div class="stat-avatar" style="background:${color}">${initials(t.name)}</div><div style="min-width:0"><div class="stat-name">${t.name}</div><div class="stat-dept">${t.dept}</div>${t.phone ? `<div class="stat-phone">${t.phone}</div>` : ''}${replaceCount > 0 ? `<div style="font-size:.68rem;color:var(--orange);font-family:var(--font-mono);margin-top:2px">🔄 ${replaceCount} запрос замены</div>` : ''}</div></div><div class="stat-nums"><div class="stat-num"><div class="stat-num-val" style="color:${color}">${monthCount}</div><div class="stat-num-label">за месяц</div></div><div class="stat-num"><div class="stat-num-val" style="color:${color}">${maxWeekLoad}/${t.maxLoad}</div><div class="stat-num-label">макс. нед.</div></div></div><div class="stat-bar"><div class="stat-bar-header"><span class="stat-bar-label">Нагрузка в неделю</span><span class="stat-bar-pct ${pctClass}">${loadPct}%</span></div><div class="stat-bar-track" role="progressbar" aria-valuenow="${loadPct}" aria-valuemin="0" aria-valuemax="100"><div class="stat-bar-fill ${barClass}" style="width:${loadPct}%;background:${barColor}"></div></div><span class="stat-status ${statusClass}">${statusLabel}</span></div><div class="stat-bar" style="margin-top:6px"><div class="stat-bar-header"><span class="stat-bar-label">Доля рабочих дней</span><span class="stat-bar-pct" style="background:${color}18;color:${color}">${monthPct}%</span></div><div class="stat-bar-track"><div class="stat-bar-fill" style="width:${monthPct}%;background:${color}"></div></div></div></div>`;
  }).join('');
}
...
function renderMyCabinet() {
  ...
  if (!tid || myDuties.length === 0) { listEl.innerHTML = `<div class="empty-state" style="padding:1.5rem"><div class="empty-icon">📅</div><p class="empty-title">Нет дежурств</p></div>`; }
    ...
    listEl.innerHTML = myDuties.map(key => { const d = new Date(key + 'T00:00:00'); const dayStr = `${d.getDate()} ${MONTHS_RU_GEN[d.getMonth()]}`; const isReq = !!State.replaceRequests[key]; return `<div class="my-duty-row"><div class="my-duty-date">${dayStr}, ${DAYS_SHORT[d.getDay()]}</div><div class="my-duty-status${isReq ? ' replace' : ''}">${isReq ? '🔄 Запрошена замена' : '✓ Запланировано'}</div><button class="my-duty-action${isReq ? ' cancel' : ''}" data-key="${key}">${isReq ? 'Отменить' : '🔄 Запросить замену'}</button></div>`; }).join('');
  ...
  if (myBlackouts.length === 0) { blEl.innerHTML = `<div style="font-family:var(--font-mono);font-size:.78rem;color:var(--text-faint);padding:.5rem 0">Нет нежелательных дат</div>`; }
  else {
...
// ─── AUTO-DISTRIBUTION (with building support and templates) ─────────────────
async function applyTemplate(templateId) {
  ...
  for (const [dayKey, dutyList] of Object.entries(dutiesObj)) { if (dayKey.startsWith(prefix)) State.duties[dayKey] = dutyList.map(d => ({ tid: d.tid, dept: d.dept || null, building: State.activeBuilding })); }
  for (const [dayKey, pairData] of Object.entries(lessonsObj)) { if (dayKey.startsWith(prefix)) { State.lessons[dayKey] = {}; for (const [pn, arr] of Object.entries(pairData)) { State.lessons[dayKey][pn] = arr.map(e => ({ tid: e.tid, dept: e.dept || '', room: e.room || '', building: State.activeBuilding })); } } }
  State.save();
  renderCalendar(); renderAccordion(); renderTeachersList(); renderStats(); renderMyCabinet();
...
async function autoDistribute(useActiveTemplate = true) {
  ...
  const prefix = `${y}-${String(m+1).padStart(2,'0')}`;
  const workdays = [];
  for (let d = 1; d <= total; d++) { const key = dateKey(y, m, d); const dow = new Date(y, m, d).getDay(); if (dow !== 0 && !getHolidayName(key)) workdays.push(key); }
  for (const wd of workdays) { const entries = getDutyEntries(wd); for (const e of entries) removeDuty(wd, e.tid, e.dept); }
  ...
  for (const key of workdays) {
    ...
    const blackoutCheck = (t) => { const bl = [...(t.blackoutDates || []), ...(State.blackoutDates[t.id] || [])]; return bl.includes(key); };
    if (!State.lessons[key]) State.lessons[key] = {};
    ...
    for (const entry of dutyPool) { const t = teacherById(entry.tid); if (!t) continue; if (blackoutCheck(t)) continue; if (monthCounts[t.id] >= 2) continue; if ((t.building || '1') !== State.activeBuilding) continue; dutyTeacher = t; break; }
    if (!dutyTeacher) { dutyTeacher = buildingTeachers.filter(t => !blackoutCheck(t) && monthCounts[t.id] < 2).sort((a, b) => monthCounts[a.id] - monthCounts[b.id])[0] || buildingTeachers[0]; }
    if (dutyTeacher) {
...
function switchTab(tab) {
  const dpanel = document.getElementById('dayPanel'); const dbackdrop = document.getElementById('dayPanelBackdrop');
...
function initTabs() {
  document.querySelectorAll('[data-tab]').forEach(btn => { btn.addEventListener('click', () => { switchTab(btn.dataset.tab); document.getElementById('mobileNav').classList.remove('open'); document.getElementById('hamburger').classList.remove('open'); document.getElementById('hamburger').setAttribute('aria-expanded', 'false'); }); });
}
...
async function initSupabase() {
  ...
  if (!sdk || typeof sdk.createClient !== 'function') { setSbStatus('error', '⚠️ SDK не загружен — проверьте интернет'); return; }
  ...
  try {
    ...
    sbReady = true;
  } catch (err) { const friendly = err.message.includes('fetch') ? '⚠️ Нет соединения с сервером — работаем офлайн' : `⚠️ ${err.message}`; console.error('[SB] Ошибка:', err.message); setSbStatus('error', friendly); sb = null; setTimeout(initSupabase, 15000); }
}
...
async function loadSchedule() { if (!sb) return; const { data, error } = await sb.from('schedule').select('date_key, teacher_id, dept, replace_request'); if (error) { console.warn('[SB] loadSchedule error:', error.message); return; } State.duties = {}; State.replaceRequests = {}; (data || []).forEach(r => { if (r.teacher_id) { if (!State.duties[r.date_key]) State.duties[r.date_key] = []; const entry = { tid: r.teacher_id, dept: r.dept || null }; const already = State.duties[r.date_key].some(e => e.tid === entry.tid && e.dept === entry.dept); if (!already) State.duties[r.date_key].push(entry); } if (r.replace_request) State.replaceRequests[r.date_key] = true; }); State.save(); renderCalendar(); renderAccordion(); renderStats(); renderMyCabinet(); }
...
async function saveTemplate() {
  ...
  for (const [key, val] of Object.entries(State.duties)) { if (!key.startsWith(prefix)) continue; const filtered = val.filter(e => e.building === building); if (filtered.length) dutiesSnapshot[key] = filtered.map(e => ({ tid: e.tid, dept: e.dept })); }
  for (const [key, pairs] of Object.entries(State.lessons)) { if (!key.startsWith(prefix)) continue; const filteredPairs = {}; for (const [pn, arr] of Object.entries(pairs)) { const filtered = arr.filter(e => e.building === building); if (filtered.length) filteredPairs[pn] = filtered.map(e => ({ tid: e.tid, dept: e.dept, room: e.room })); } if (Object.keys(filteredPairs).length) lessonsSnapshot[key] = filteredPairs; }
  ...
  if (existing && existing.length) { if (!confirm(`Шаблон «${templateName}» уже существует. Перезаписать?`)) return; await sb.from('templates').update({ duties_json: dutiesSnapshot, lessons_json: lessonsSnapshot, created_at: new Date() }).eq('id', existing[0].id); showToast(`Шаблон «${templateName}» обновлён`, 'success'); }
  else { const { error } = await sb.from('templates').insert({ name: templateName, building, duties_json: dutiesSnapshot, lessons_json: lessonsSnapshot }); if (error) showToast('Ошибка сохранения шаблона', 'error'); else showToast(`Шаблон «${templateName}» сохранён`, 'success'); }
  loadTemplatesList();
}
...
function renderTemplatesList(templates = null) {
  ...
  if (select) { select.innerHTML = '<option value="">— Без шаблона (случайное) —</option>' + templates.map(t => `<option value="${t.id}" ${State.activeTemplateId == t.id ? 'selected' : ''}>${t.name}</option>`).join(''); select.onchange = (e) => { const id = e.target.value; State.activeTemplateId = id || null; State.save(); renderTemplatesList(templates); }; }
  ...
  container.innerHTML = templates.map(t => `<div class="template-tag ${State.activeTemplateId === t.id ? 'active' : ''}" data-id="${t.id}"><span>${t.name}</span><button class="template-remove" data-id="${t.id}" title="Удалить шаблон">✕</button></div>`).join('');
  container.querySelectorAll('.template-tag[data-id]').forEach(tag => { tag.addEventListener('click', (e) => { if (e.target.classList.contains('template-remove')) return; const tid = tag.dataset.id; if (State.activeTemplateId === tid) { State.activeTemplateId = null; tag.classList.remove('active'); if (select) select.value = ''; showToast('Активный шаблон снят', 'info'); } else { State.activeTemplateId = tid; if (select) select.value = tid; document.querySelectorAll('.template-tag').forEach(t => t.classList.remove('active')); tag.classList.add('active'); showToast(`Активный шаблон: ${tag.querySelector('span').textContent}`, 'success'); } State.save(); }); const delBtn = tag.querySelector('.template-remove'); if (delBtn) { delBtn.addEventListener('click', async (e) => { e.stopPropagation(); const tid = delBtn.dataset.id; if (confirm('Удалить шаблон?')) { if (sb) await sb.from('templates').delete().eq('id', tid); if (State.activeTemplateId === tid) State.activeTemplateId = null; loadTemplatesList(); showToast('Шаблон удалён', 'info'); } }); } });
}
