/* ═══════════════════════════════════════════════════════════════════════════
   ...
   Изменения:
   - фиолетовый цвет для дежурных (#8E44AD), красный для blackout (#C0392B)
   - полная независимость корпусов: schedule и lessons привязаны к building,
     все операции (добавление, удаление, авто-распределение,
     шаблоны) работают строго в рамках выбранного корпуса.
   - система шаблонов расстановки (завуч): сохранение/загрузка/удаление,
     выбор активного шаблона для авто-распределения.
   ═══════════════════════════════════════════════════════════════════════════ */
...
const State = {
  teachers: [],
  duties: {},            // key → [{ tid, dept, building }]
  replaceRequests: {},   // key → true
  blackoutDates: {},
  notifications: [],
  lessons: {},           // key → { pairNum → [{ tid, dept, room, building }] }
  currentRole: 'admin',
  ...
  modalMode: 'assign',
  activeDayKey: null,
  activeBuilding: '1',   // '1' | '2' | '3'
  activeTemplateId: null, // id выбранного шаблона для автозагрузки
...
// ─── MULTI-DUTY HELPERS (с учётом building) ──────────────────────────────────
function normEntry(entry) {
  if (typeof entry === 'string') return { tid: entry, dept: null, building: State.activeBuilding };
  return { tid: entry.tid || entry.id || '', dept: entry.dept || null, building: entry.building || State.activeBuilding };
}
...
function applyRole(role) {
  ...
  if (role === 'admin') {
    if (!State.activeBuilding) State.activeBuilding = '1';
    _syncBuildingTabs(State.activeBuilding);
    renderTemplatesList();   // обновить шаблоны
  }
  if (role === 'teacher') {
    if (!State.currentTeacherId && State.teachers.length) State.currentTeacherId = State.teachers[0].id;
...
function addNotification(msg, icon='🔔') {
  State.notifications.unshift({ id:'n_'+Date.now(), msg, icon, time: new Date().toLocaleTimeString('ru-RU', {hour:'2-digit',minute:'2-digit'}) });
...
function renderCalendar() {
  ...
  for (let d=1; d<=lastDay.getDate(); d++) {
    ...
    if (isSunday) {
      ...
      const label = document.createElement('div'); label.className='holiday-label'; label.textContent='Выходной';
    ...
    // Получаем дежурства с фильтром по building
    const dutyEntries = getDutyEntries(key);
    // Получаем пары для текущего корпуса
    const pairTeacherIds = new Set();
    if (State.lessons[key]) {
      for (let pn=1; pn<=6; pn++) {
        const arr = State.lessons[key][pn] || [];
        arr.forEach(e => { if (e.building === State.activeBuilding) pairTeacherIds.add(e.tid); });
      }
    }
    ...
    if (!isHoliday) {
      if (State.currentRole === 'teacher') {
        ...
        if (hasLesson || isDuty) {
          ...
          if (hasLesson) { const s=document.createElement('div'); s.className='cell-strip cell-strip--lesson'; stripWrap.appendChild(s); }
          if (isDuty)    { const s=document.createElement('div'); s.className='cell-strip cell-strip--duty'; stripWrap.appendChild(s); }
          cell.appendChild(stripWrap);
        }
      } else {
        // Admin: показываем аватар+фио дежурных только из текущего корпуса
...
// ─── MOBILE ACCORDION (аналогичный фильтр по building) ──────────────────────
function renderAccordion() {
  ...
  weeks.forEach((week, wi) => {
    ...
    const assignedCount = week.filter(({key, dow}) => getDutyIds(key).length>0 && dow!==0 && dow!==6 && !getHolidayName(key)).length;
    ...
    week.forEach(({d, key, dow}) => {
      if (dow===0) return; // воскресенье пропускаем
      const isHoliday = !!getHolidayName(key);
      const isToday   = key===today;
      ...
      if (isHoliday) {
        ...
        contentHtml = dutyEntries.map(entry => {
          ...
          const replaceBadge = (isReplace && isMyDutyHere) ? '<div style="font-size:.68rem;color:var(--warning);margin-top:3px">🔄 Просит замену</div>' : '';
          const removeBtn = (State.currentRole==='admin')
            ? `<button class="acc-remove-duty mob-only" data-remove-tid="${entry.tid}" data-remove-dept="${entry.dept||''}" data-remove-key="${key}">✕</button>`
            : '';
          return `<div class="acc-duty-chip${isReplace && isMyDutyHere ? ' replace' : ''}" style="border-left-color:${color};background:${color}12;border-color:${color}40;margin-bottom:4px;position:relative">
            ${removeBtn}
            <div class="acc-duty-name">${t.name}</div><div class="acc-duty-dept">${entry.dept||t.dept||''}</div>
            ${t.phone ? `<div class="acc-duty-dept">${t.phone}</div>` : ''}
            ${replaceBadge}
          </div>`;
        }).join('');
      } else {
        contentHtml = `<div class="acc-empty">— свободно —</div>`;
      }
      ...
      if (btn) {
        btn.addEventListener('click', e => {
          ...
          if (act==='assign' || act==='replace') openModal(k, kd, km-1, ky, act);
          else handleChipAction(act, k);
        });
      }
...
function openModal(key, day, month, year, mode='assign') {
  ...
  if (assignedEntries.length) {
    ...
    assignedEntries.forEach(entry => {
      ...
      html += `<div class="modal-assigned-row">
        <div class="opt-avatar" style="background:${color};width:28px;height:28px;font-size:.7rem;cursor:pointer" onclick="openTeacherInfoModal('${entry.tid}')">${initials(t.name)}</div>
        <div style="flex:1"><span class="modal-assigned-name">${t.name.split(' ').slice(0,2).join(' ')}</span>${entry.dept ? `<span style="display:block;font-size:.7rem;color:var(--text-muted)">${entry.dept}</span>` : ''}</div>
        <button class="modal-remove-one" data-tid="${entry.tid}" data-dept="${entry.dept||''}">✕</button>
      </div>`;
    });
  ...
  if (State.teachers.length===0) {
    html += `<div class="empty-state"><div class="empty-icon">👤</div><p>Нет преподавателей</p></div>`;
  } else {
    ...
    State.teachers.forEach((t, idx) => {
      ...
      const deptSelector = depts.length>1
        ? `<select class="dept-sel" data-tid="${t.id}" style="font-size:.7rem;border:1px solid var(--border);border-radius:4px;padding:2px 4px;margin-top:3px;">${depts.map(d=>`<option>${d}</option>`).join('')}</select>`
        : `<span style="font-size:.72rem;color:var(--text-muted)">${depts[0]||''}</span>`;
      html += `<button class="teacher-option" data-id="${t.id}">
        <div class="opt-avatar" style="background:${color}" onclick="event.stopPropagation();openTeacherInfoModal('${t.id}')">${initials(t.name)}</div>
        <div class="opt-info"><div class="opt-name">${t.name}</div><div class="opt-meta">нед: ${wc}/${t.maxLoad}${t.phone ? ' · '+t.phone : ''}</div>${deptSelector}</div>${badge}
      </button>`;
    });
    html += `</div>`;
  }
  ...
  body.querySelectorAll('.teacher-option').forEach(btn => {
    btn.addEventListener('click', () => {
      body.querySelectorAll('.teacher-option').forEach(b=>b.classList.remove('selected'));
...
function saveModal() {
  ...
  if (State.selectedTeacherId) {
    ...
    addDuty(key, State.selectedTeacherId, chosenDept, State.activeBuilding);
...
// ─── DAY DETAIL PANEL (с учётом building) ───────────────────────────────────
...
function closeDayPanel() {
  ...
  document.getElementById('dayPanelBackdrop')?.classList.remove('open');
...
function renderDayPanel(key) {
  ...
  if (dutyEntries.length) {
    dutyStrip.innerHTML = `<div style="font-size:.72rem;color:var(--text-muted);margin-bottom:6px">Дежурные</div><div style="display:flex;flex-wrap:wrap;gap:6px;">` +
      dutyEntries.map(e => {
        ...
        return `<div style="display:flex;align-items:center;gap:5px;background:${color}14;border:1px solid ${color}40;border-radius:20px;padding:3px 10px 3px 4px;cursor:pointer" onclick="openTeacherInfoModal('${e.tid}')">
          <div style="width:22px;height:22px;border-radius:50%;background:${color};display:flex;align-items:center;justify-content:center;font-size:.58rem;font-weight:700;color:#fff;">${initials(t.name)}</div>
          <span style="font-size:.75rem;font-weight:500">${t.name.split(' ').slice(0,2).join(' ')}</span>
          ${isAdmin ? `<button onclick="event.stopPropagation();removeDutyFromPanel('${key}','${e.tid}','${e.dept||''}')" style="background:none;border:none;color:var(--text-faint);cursor:pointer;">✕</button>` : ''}
        </div>`;
      }).join('') + `</div>${isAdmin ? `<button class="day-panel-add-btn" onclick="openModal('${key}',${dd},${mm-1},${y},'assign')">+ Добавить дежурного</button>` : ''}`;
  } else {
    dutyStrip.innerHTML = isHoliday
      ? `<div class="day-panel-holiday">🏛 ${getHolidayName(key)}</div>`
      : `<div style="font-size:.82rem;color:var(--text-faint);font-style:italic">Дежурных не назначено</div>${isAdmin ? `<button class="day-panel-add-btn" onclick="openModal('${key}',${dd},${mm-1},${y},'assign')">+ Назначить дежурного</button>` : ''}`;
  }
  ...
  pairsEl.innerHTML = PAIRS.map(p => {
    ...
    const entriesHtml = entries.length
      ? entries.map((e,i) => {
          ...
          return `<div class="pair-teacher-row">
            <div style="width:28px;height:28px;border-radius:50%;background:${color};display:flex;align-items:center;justify-content:center;font-size:.65rem;font-weight:700;color:#fff;cursor:pointer" onclick="openTeacherInfoModal('${e.tid}')">${initials(t.name)}</div>
            <div style="flex:1"><div style="font-size:.82rem;font-weight:600">${t.name.split(' ').slice(0,2).join(' ')}</div><div style="font-size:.7rem;color:var(--text-muted)">${e.dept||t.dept||''}${e.room ? ' · 🚪 '+e.room : ''}</div></div>
            ${isAdmin ? `<button class="pair-remove-btn" onclick="removePairEntry('${key}',${p.n},${i})">✕</button>` : ''}
          </div>`;
        }).join('')
    ...
    return `<details class="pair-block" ${entries.length ? 'open' : ''}>
      <summary class="pair-summary">
        <span class="pair-num">Пара ${p.n}</span><span class="pair-time">${p.time}</span>
        <span class="pair-count">${entries.length ? entries.length+' преп.' : ''}</span><span class="pair-chevron">▾</span>
      </summary>
      <div class="pair-body">
        <div class="pair-teachers-list" id="pair-list-${key}-${p.n}">${entriesHtml}</div>
        ${isAdmin ? `<div class="pair-add-row">
          <div class="select-wrap" style="flex:1"><select class="field-input field-select pair-teacher-sel" style="height:32px;font-size:.78rem" id="pair-tsel-${key}-${p.n}" onchange="updatePairDeptSel('${key}',${p.n})"><option value="">— Преподаватель —</option>${State.teachers.map(t=>`<option value="${t.id}">${t.name.split(' ').slice(0,2).join(' ')}</option>`).join('')}</select></div>
          <div id="pair-dept-wrap-${key}-${p.n}" style="display:none"><select class="field-input field-select" id="pair-dept-${key}-${p.n}" style="height:32px;font-size:.75rem"></select></div>
...
function addPairEntry(key, pairN) {
  ...
  if (typeof saveLessonsBatch === 'function') saveLessonsBatch(key, State.lessons[key]);
}
function removePairEntry(key, pairN, idx) {
  const entries = State.lessons[key]?.[pairN] || [];
...
// ─── DEPARTMENT MANAGER (глобальные кафедры) ────────────────────────────────
const DEFAULT_DEPTS = ['Кафедра информатики и ВТ','Кафедра математики','Кафедра физики','Кафедра химии и биологии',
  'Кафедра истории и обществознания','Кафедра русского языка и литературы','Кафедра иностранных языков',
  'Кафедра физической культуры','Кафедра экономики и права','Кафедра психологии и педагогики'];
...
function renderDeptManager(selected) {
  ...
  const selectedList = selected.length
    ? selected.map((d,i)=>`<div style="display:flex;gap:6px;margin-bottom:4px"><input class="field-input" style="flex:1;height:32px;font-size:.8rem" value="${d.replace(/"/g,'&quot;')}" data-dept-i="${i}"/><button class="modal-remove-one" data-dept-del="${i}">✕</button></div>`).join('')
    : '<div style="font-size:.78rem;color:var(--text-faint);padding:4px 0">Нет кафедр</div>';
  container.innerHTML = `<label class="field-label">Кафедры <span class="required">*</span></label><div id="tDeptsList">${selectedList}</div>
    <div style="display:flex;gap:6px;margin-top:6px;align-items:center;flex-wrap:wrap">
      <div class="select-wrap" style="flex:1"><select class="field-input field-select" id="tDeptPickSel" style="height:34px">${globalDepts.map(d=>`<option>${d}</option>`).join('')}</select></div>
      <button class="btn btn--outline" id="tDeptAddFromList" style="padding:.3rem .7rem">+ Добавить</button>
    </div>
    <div style="display:flex;gap:6px;margin-top:4px"><input class="field-input" id="tDeptNewInput" placeholder="Новая кафедра…" style="flex:1;height:32px"/><button class="btn btn--outline" id="tDeptAddNew" style="padding:.3rem .7rem">+ Создать</button></div>`;
  container.querySelectorAll('[data-dept-i]').forEach(inp=>inp.addEventListener('change',()=>{ _modalDepts[parseInt(inp.dataset.deptI)]=inp.value.trim(); }));
  ...
  document.getElementById('tDeptAddFromList').addEventListener('click',()=>{ const v=document.getElementById('tDeptPickSel').value; if(v && !_modalDepts.includes(v)){ _modalDepts.push(v); renderDeptManager(_modalDepts); } else showToast('Уже добавлено','info'); });
  document.getElementById('tDeptAddNew').addEventListener('click',()=>{ const v=document.getElementById('tDeptNewInput').value.trim(); if(v){ if(!globalDepts.includes(v)){ globalDepts.push(v); saveGlobalDepts(globalDepts); } if(!_modalDepts.includes(v)){ _modalDepts.push(v); renderDeptManager(_modalDepts); } else showToast('Уже добавлено','info'); document.getElementById('tDeptNewInput').value=''; } });
}
...
function openTeacherInfoModal(tid) {
  ...
  const depts = Array.isArray(t.depts)&&t.depts.length ? t.depts : [t.dept].filter(Boolean);
  const deptsHtml = depts.map(d=>`<span style="background:${color}18;border:1px solid ${color}40;border-radius:12px;padding:3px 10px;font-size:.78rem;margin:2px">${d}</span>`).join('');
  ...
  document.getElementById('teacherInfoBody').innerHTML = `
    <div style="display:flex;align-items:center;gap:14px;margin-bottom:1.25rem">
      <div style="width:56px;height:56px;border-radius:50%;background:${color};display:flex;align-items:center;justify-content:center;font-size:1.2rem;font-weight:700;color:#fff;">${initials(t.name)}</div>
      <div><div style="font-size:1.05rem;font-weight:700">${t.name}</div>${t.phone?`<div style="font-size:.82rem;color:var(--text-muted);margin-top:3px">📞 ${t.phone}</div>`:''}</div>
    </div>
    <div style="margin-bottom:.75rem"><div class="modal-section-label" style="margin-bottom:6px">Кафедры</div><div>${deptsHtml||'—'}</div></div>
    <div style="display:flex;gap:1rem;margin-bottom:.75rem"><div style="flex:1;background:var(--surface-2);border-radius:var(--radius-sm);padding:10px;text-align:center"><div style="font-size:1.5rem;font-weight:700;color:${color}">${dutyCount}</div><div style="font-size:.7rem;color:var(--text-muted)">дежурств в месяце</div></div>
    <div style="flex:1;background:var(--surface-2);border-radius:var(--radius-sm);padding:10px;text-align:center"><div style="font-size:1.5rem;font-weight:700;color:${color}">${t.maxLoad}</div><div style="font-size:.7rem;color:var(--text-muted)">макс. дн/нед</div></div></div>
    ${blackouts.length?`<div><div class="modal-section-label" style="margin-bottom:6px">🚫 Нежелательные даты</div><div style="font-size:.8rem;color:var(--text-secondary);font-family:var(--font-mono)">${blackouts.join(', ')}</div></div>`:''}
    <div class="modal-actions" style="margin-top:1.25rem"><button class="btn-modal-clear" onclick="closeModal('teacherInfoOverlay')">Закрыть</button>${State.currentRole==='admin'?`<button class="btn-modal-save" onclick="closeModal('teacherInfoOverlay');openTeacherModal('${tid}')">✏️ Редактировать</button>`:''}</div>`;
  document.getElementById('teacherInfoOverlay').classList.add('open');
}
...
function renderModalBlackouts(dates, tid) {
  ...
  const tagsHtml = sorted.length ? sorted.map(k=>{ const d=new Date(k+'T00:00:00'); return `<span class="blackout-tag">${d.getDate()} ${MONTHS_RU_GEN[d.getMonth()]} ${d.getFullYear()}<button class="blackout-remove" data-k="${k}">✕</button></span>`; }).join('') : '<span style="font-size:.78rem;color:var(--text-faint)">Нет нежелательных дат</span>';
  container.innerHTML = `<div class="field-group field-group--wide" style="margin-top:.75rem"><label class="field-label">🚫 Нежелательные даты (алгоритм пропустит их)</label>
    <div style="display:flex;gap:.5rem;align-items:center;margin-bottom:.5rem"><input type="date" class="field-input" id="tBlackoutInput" style="flex:1"/><button class="btn btn--outline" id="tBlackoutAddBtn">+ Добавить</button></div>
    <div id="tBlackoutTags" style="display:flex;flex-wrap:wrap;gap:.35rem;">${tagsHtml}</div></div>`;
  document.getElementById('tBlackoutAddBtn').addEventListener('click',()=>{ const v=document.getElementById('tBlackoutInput').value; if(v && !_modalBlackouts.includes(v)){ _modalBlackouts.push(v); renderModalBlackouts(_modalBlackouts, tid); } else if(v) showToast('Уже добавлено','info'); });
...
function openTeacherModal(editId=null) {
  ...
  const existing = editId ? (State.blackoutDates[editId]||[]) : [];
  _modalBlackouts = [...existing];
...
function saveTeacherModal() {
  ...
  document.querySelectorAll('[data-dept-i]').forEach(inp=>{ _modalDepts[parseInt(inp.dataset.deptI)] = inp.value.trim(); });
  ...
  const building = document.getElementById('tBuilding').value;
...
function renderTeachersList() {
  ...
  if(!State.teachers.length){
    container.innerHTML = `<div class="empty-state"><div class="empty-icon">📋</div><p>Список пуст</p></div>`;
    return;
  }
  const byBuilding = {'1':[],'2':[],'3':[]};
  State.teachers.forEach((t,idx)=>{ const b=t.building||'1'; if(byBuilding[b]) byBuilding[b].push({t,idx}); else byBuilding[b]=[{t,idx}]; });
  container.innerHTML = ['1','2','3'].map(b=>`
    <details class="building-section" open>
      <summary class="building-section-header"><span class="building-section-label">🏢 ${b} корпус</span><span class="building-section-count">${byBuilding[b].length} преп.</span><span class="building-section-chevron">▾</span></summary>
      <div class="building-section-table">${byBuilding[b].length===0 ? '<div class="building-empty">Нет преподавателей</div>' :
        `<table class="teacher-table"><thead><tr><th>ФИО</th><th>Кафедра</th><th>Телефон</th><th style="text-align:center">Нагр.</th><th style="text-align:center">Дн.</th><th></th></tr></thead><tbody>${byBuilding[b].map(({t,idx})=>{
          const color=getColor(idx); const dc=getMonthDutyCount(t.id);
          return `<tr><td><div style="display:flex;align-items:center;gap:8px"><div style="width:28px;height:28px;border-radius:50%;background:${color};display:flex;align-items:center;justify-content:center;font-size:.62rem;font-weight:700;color:#fff;">${initials(t.name)}</div><span style="font-size:.82rem;font-weight:600">${t.name}</span></div></td>
          <td style="font-size:.75rem;color:var(--text-muted)">${t.dept}</td><td style="font-size:.75rem;font-family:var(--font-mono)">${t.phone||'—'}</td>
          <td style="text-align:center;font-size:.78rem">${t.maxLoad}</td><td style="text-align:center;font-size:.78rem;font-weight:600;color:${dc>0?'var(--purple)':'var(--text-faint)'}">${dc}</td>
          <td><div style="display:flex;gap:4px"><button class="t-remove" style="padding:4px 8px;border:1px solid var(--border);border-radius:var(--radius-sm);background:var(--surface);cursor:pointer" data-edit="${t.id}">✏️</button>
          <button class="t-remove" data-id="${t.id}"><svg viewBox="0 0 16 16" fill="none" width="13" height="13"><path d="M3 4h10M6 4V2.5h4V4M5.5 4l.5 9M10.5 4l-.5 9" stroke="currentColor" stroke-width="1.4" stroke-linecap="round"/></svg></button></div></td></tr>`;
...
function removeTeacher(id){
  ...
  Object.keys(State.duties).forEach(k=>{
    ...
    if(entries.length) State.duties[k]=entries; else delete State.duties[k];
  });
...
function renderInlineDeptManager(depts){
  ...
  if(!container) return;
  const tagsHtml = depts.length ? depts.map((d,i)=>`<div style="display:flex;align-items:center;gap:4px;margin-bottom:3px"><span style="flex:1;font-size:.78rem;background:var(--blue-light);border:1px solid var(--blue);border-radius:4px;padding:2px 8px;">${d}</span><button class="modal-remove-one" data-inline-del="${i}">✕</button></div>`).join('') : '<div style="font-size:.75rem;color:var(--text-faint)">Нет кафедр</div>';
  container.innerHTML = `<div id="inlineDeptTags" style="margin-bottom:6px">${tagsHtml}</div>
    <div style="display:flex;gap:5px;margin-bottom:4px"><div class="select-wrap" style="flex:1"><select class="field-input field-select" id="inlineDeptPickSel" style="height:32px">${globalDepts.map(d=>`<option>${d}</option>`).join('')}</select></div><button class="btn btn--outline" id="inlineDeptAddFromList" style="padding:.3rem .7rem">+</button></div>
    <div style="display:flex;gap:5px"><input class="field-input" id="inlineDeptNewInput" placeholder="Новая кафедра…" style="flex:1;height:32px"/><button class="btn btn--outline" id="inlineDeptAddNew" style="padding:.3rem .7rem">+ Создать</button></div>`;
  ...
  document.getElementById('inlineDeptAddFromList')?.addEventListener('click',()=>{ const v=document.getElementById('inlineDeptPickSel').value; if(v && !_inlineDepts.includes(v)){ _inlineDepts.push(v); renderInlineDeptManager(_inlineDepts); } });
  document.getElementById('inlineDeptAddNew')?.addEventListener('click',()=>{ const v=document.getElementById('inlineDeptNewInput').value.trim(); if(v){ if(!globalDepts.includes(v)){ globalDepts.push(v); saveGlobalDepts(globalDepts); } if(!_inlineDepts.includes(v)){ _inlineDepts.push(v); renderInlineDeptManager(_inlineDepts); } document.getElementById('inlineDeptNewInput').value=''; } });
}
function addTeacher() {
...
function seedDemoData(){
  ...
  sb.from('teachers').upsert(demoTeachers,{onConflict:'id'}).then(({error})=>{ if(error) showToast('Ошибка загрузки демо','error'); else { showToast('10 демо-преподавателей добавлены','success'); loadTeachers(); } });
}
...
function renderStats() {
  ...
  if(!grid) return;
  if(!State.teachers.length){
    grid.innerHTML = `<div class="empty-state"><div class="empty-icon">📊</div><p>Нет данных</p></div>`;
    return;
  }
  const y=State.currentDate.getFullYear(); const m=State.currentDate.getMonth();
  const prefix=`${y}-${String(m+1).padStart(2,'0')}`;
  ...
  grid.innerHTML = State.teachers.map((t,idx)=>{
    ...
    return `<div class="stat-card"><div class="stat-header"><div class="stat-avatar" style="background:${color}">${initials(t.name)}</div><div><div class="stat-name">${t.name}</div><div class="stat-dept">${t.dept}</div>${t.phone?`<div class="stat-phone">${t.phone}</div>`:''}${replaceCount?`<div style="font-size:.68rem;color:var(--warning);margin-top:2px">🔄 ${replaceCount} запрос замены</div>`:''}</div></div>
    <div class="stat-nums"><div class="stat-num"><div class="stat-num-val" style="color:${color}">${monthCount}</div><div class="stat-num-label">за месяц</div></div><div class="stat-num"><div class="stat-num-val" style="color:${color}">${maxWeekLoad}/${t.maxLoad}</div><div class="stat-num-label">макс. нед.</div></div></div>
    <div class="stat-bar"><div class="stat-bar-header"><span class="stat-bar-label">Нагрузка в неделю</span><span class="stat-bar-pct stat-bar-pct--${loadStatus}">${loadPct}%</span></div><div class="stat-bar-track"><div class="stat-bar-fill ${barClass}" style="width:${loadPct}%;background:${barColor}"></div></div><span class="stat-status stat-status--${loadStatus}">${statusLabel}</span></div>
    <div class="stat-bar" style="margin-top:6px"><div class="stat-bar-header"><span class="stat-bar-label">Доля рабочих дней</span><span class="stat-bar-pct" style="background:${color}18;color:${color}">${monthPct}%</span></div><div class="stat-bar-track"><div class="stat-bar-fill" style="width:${monthPct}%;background:${color}"></div></div></div></div>`;
  }).join('');
}
...
function renderMyCabinet() {
  ...
  const cabName = document.getElementById('cabinetTeacherName');
  if(cabName){ const t=teacherById(tid); cabName.textContent=t?t.name:'—'; }
  const y=State.currentDate.getFullYear(); const m=State.currentDate.getMonth();
  const prefix=`${y}-${String(m+1).padStart(2,'0')}`;
  const myDuties = Object.keys(State.duties).filter(k=>k.startsWith(prefix) && getDutyEntries(k).some(e=>e.tid===tid)).sort();
  if(!tid || myDuties.length===0){
    listEl.innerHTML = `<div class="empty-state"><div class="empty-icon">📅</div><p>Нет дежурств</p></div>`;
  } else {
    listEl.innerHTML = myDuties.map(key=>{
      const d = new Date(key+'T00:00:00');
      const dayStr = `${d.getDate()} ${MONTHS_RU_GEN[d.getMonth()]}`;
      const isReq = !!State.replaceRequests[key];
      return `<div class="my-duty-row"><div class="my-duty-date">${dayStr}, ${DAYS_SHORT[d.getDay()]}</div><div class="my-duty-status${isReq?' replace':''}">${isReq?'🔄 Запрошена замена':'✓ Запланировано'}</div><button class="my-duty-action${isReq?' cancel':''}" data-key="${key}">${isReq?'Отменить':'🔄 Запросить замену'}</button></div>`;
    }).join('');
  ...
  if(myBlackouts.length===0){
    blEl.innerHTML = `<div style="font-size:.78rem;color:var(--text-faint);padding:.5rem 0">Нет нежелательных дат</div>`;
  } else {
    blEl.innerHTML = myBlackouts.map(k=>{
      const d=new Date(k+'T00:00:00');
      return `<div class="blackout-tag">${d.getDate()} ${MONTHS_RU_GEN[d.getMonth()]} ${d.getFullYear()} (${DAYS_FULL[d.getDay()]})<button class="blackout-remove" data-k="${k}">✕</button></div>`;
    }).join('');
    blEl.querySelectorAll('.blackout-remove').forEach(btn=>btn.addEventListener('click',()=>{
      State.blackoutDates[tid] = (State.blackoutDates[tid]||[]).filter(k=>k!==btn.dataset.k);
      State.save();
...
async function applyTemplate(templateId) {
  ...
  const m = State.currentDate.getMonth();
  const prefix = `${y}-${String(m+1).padStart(2,'0')}`;
  // Очищаем текущий месяц для этого корпуса
...
async function autoDistribute(useActiveTemplate = true) {
  ...
  // Очищаем только для текущего корпуса, оставляя другие корпуса нетронутыми
  for(const wd of workdays){
    const entries = getDutyEntries(wd); // уже фильтруется по building
    for(const e of entries) removeDuty(wd, e.tid, e.dept);
  }
  ...
  // Фильтр преподавателей по текущему корпусу
  ...
  for(const key of workdays){
    ...
    if(!dutyTeacher){
      dutyTeacher = buildingTeachers.filter(t=>!blackoutCheck(t) && monthCounts[t.id]<2).sort((a,b)=>monthCounts[a.id]-monthCounts[b.id])[0] || buildingTeachers[0];
    }
    if(dutyTeacher){
...
async function saveTemplate() {
  ...
  // Собираем duties и lessons только для текущего корпуса и текущего месяца
  const y = State.currentDate.getFullYear();
  const m = State.currentDate.getMonth();
  const prefix = `${y}-${String(m+1).padStart(2,'0')}`;
  ...
  // Проверка на существование шаблона с таким именем и корпусом
...
async function loadTemplatesList() {
  if(!sb) { renderTemplatesList([]); return; }
  const { data, error } = await sb.from('templates').select('*').eq('building', State.activeBuilding).order('name');
  if(error) { console.warn(error); renderTemplatesList([]); return; }
...
function renderTemplatesList(templates = null) {
  ...
  if(templates.length===0){
    container.innerHTML = '<div style="font-size:.78rem;color:var(--text-faint);padding:.25rem 0">Нет шаблонов</div>';
    return;
  }
...
function switchTab(tab){
  ...
  document.querySelectorAll('.nav-btn, .mob-nav-btn').forEach(b=>b.classList.toggle('active', b.dataset.tab===tab));
...
async function initSupabase(){
  ...
  try{
    ...
    await sb.from('teachers').select('id').limit(1);
    await loadTeachers();
    await loadSchedule();
    await loadLessons();
    subscribeRealtime();
    ...
    setSbStatus('error',err.message.includes('fetch') ? 'нет сети' : 'ошибка БД');
    sb=null;
...
async function loadSchedule(){
  if(!sb) return;
  const {data,error}=await sb.from('schedule').select('date_key, teacher_id, dept, replace_request, building');
  if(error){ console.warn(error); return; }
  State.duties={};
  ...
  (data||[]).forEach(r=>{
    if(r.teacher_id){
      ...
      const entry = { tid:r.teacher_id, dept:r.dept||null, building:r.building||'1' };
      if(!State.duties[r.date_key].some(e=>e.tid===entry.tid && e.dept===entry.dept && e.building===entry.building))
        State.duties[r.date_key].push(entry);
    }
    if(r.replace_request) State.replaceRequests[r.date_key]=true;
  });
  State.save();
  renderCalendar(); renderAccordion(); renderStats(); renderMyCabinet();
}
async function loadLessons(){
  if(!sb) return;
  const y=State.currentDate.getFullYear(); const m=State.currentDate.getMonth();
  const prefix=`${y}-${String(m+1).padStart(2,'0')}%`;
  const {data,error}=await sb.from('lessons').select('date_key,pair_num,teacher_id,dept,room,building').like('date_key',prefix);
  if(error){ console.warn(error); return; }
  Object.keys(State.lessons).forEach(k=>{ if(k.startsWith(prefix.slice(0,-1))) delete State.lessons[k]; });
  (data||[]).forEach(r=>{
    if(!State.lessons[r.date_key]) State.lessons[r.date_key]={};
    if(!State.lessons[r.date_key][r.pair_num]) State.lessons[r.date_key][r.pair_num]=[];
    State.lessons[r.date_key][r.pair_num].push({ tid:r.teacher_id, dept:r.dept||'', room:r.room||'', building:r.building||'1' });
  });
}
function mapTeacherRow(r){
  let depts=[];
  if(Array.isArray(r.depts) && r.depts.length) depts=r.depts;
  else if(r.dept) depts=[r.dept];
  return {
    id: r.id, name: r.name, dept: depts[0]||'', depts: depts,
    phone: r.phone||'', maxLoad: r.max_load||2,
...
async function saveTeachers(teacher){
  ...
  await sb.from('teachers').upsert({
    id: teacher.id, name: teacher.name, dept: depts[0]||'', depts: depts,
    phone: teacher.phone||'', max_load: teacher.maxLoad||2,
...
async function saveSchedule(key, teacherId, replaceRequest=false, dept=null, building=State.activeBuilding){
  if(!sb) return;
  if(teacherId){
    await sb.from('schedule').upsert({ date_key:key, teacher_id:teacherId, dept:dept||null, replace_request:replaceRequest, building:building },{ onConflict:'date_key,teacher_id,dept' });
  } else {
...
async function saveScheduleRemoveOne(key, teacherId){
  if(!sb) return;
  await sb.from('schedule').delete().eq('date_key',key).eq('teacher_id',teacherId);
}
...
async function deleteScheduleMonth(year, month){
  ...
  const to   = `${prefix}-32`;
  await sb.from('schedule').delete().gte('date_key',from).lte('date_key',to);
}
async function saveLessonsBatch(key, lessons){
  ...
  for(let pn=1;pn<=6;pn++){
    (lessons[pn]||[]).forEach(e=>{
      rows.push({ date_key:key, pair_num:pn, teacher_id:e.tid, dept:e.dept||'', room:e.room||'', building:e.building||State.activeBuilding });
    });
  }
  if(rows.length) await sb.from('lessons').insert(rows);
}
function subscribeRealtime(){
  if(!sb || sbChannel) return;
  sbChannel = sb.channel('ag-realtime-v6')
    .on('postgres_changes',{event:'*',schema:'public',table:'schedule'}, onScheduleChange)
    .on('postgres_changes',{event:'*',schema:'public',table:'teachers'}, onTeacherChange)
    .on('postgres_changes',{event:'*',schema:'public',table:'lessons'}, onLessonsChange)
    .subscribe(status=>{
      if(status==='SUBSCRIBED') setSbStatus('connected','подключено · Realtime ⚡');
      else if(status==='CHANNEL_ERROR'||status==='TIMED_OUT') setTimeout(()=>{ sbChannel=null; subscribeRealtime(); },5000);
    });
}
function onScheduleChange({eventType, new:row, old:oldRow}){
  if(_suppressRealtimeRender) return;
  ...
  if(eventType==='DELETE'){
    ...
    if(row.teacher_id) addDuty(key, row.teacher_id, row.dept||null, row.building||'1');
    if(row.replace_request) State.replaceRequests[key]=true;
    else delete State.replaceRequests[key];
  }
  State.save();
  renderCalendar(); renderAccordion(); renderTeachersList(); renderStats(); renderMyCabinet();
}
function onTeacherChange({eventType, new:row}){
  if(eventType==='DELETE') State.teachers = State.teachers.filter(t=>t.id!==row.id);
  else {
    ...
    if(mapped.blackoutDates?.length) State.blackoutDates[mapped.id]=mapped.blackoutDates;
  }
  State.save();
  renderTeachersList(); renderCalendar(); renderAccordion(); renderStats();
}
function onLessonsChange({eventType, new:row}){
  if(_suppressRealtimeRender) return;
  const key = row?.date_key;
  if(!key) return;
  if(eventType==='DELETE'){
    // перезагрузим день
    if(sb) sb.from('lessons').select('*').eq('date_key',key).then(({data})=>{
      State.lessons[key]={};
      (data||[]).forEach(r=>{
        if(!State.lessons[key][r.pair_num]) State.lessons[key][r.pair_num]=[];
        State.lessons[key][r.pair_num].push({ tid:r.teacher_id, dept:r.dept||'', room:r.room||'', building:r.building||'1' });
      });
    ...
    if(!State.lessons[key]) State.lessons[key]={};
    if(!State.lessons[key][row.pair_num]) State.lessons[key][row.pair_num]=[];
    const exists = State.lessons[key][row.pair_num].some(e=>e.tid===row.teacher_id && e.room===row.room);
    if(!exists) State.lessons[key][row.pair_num].push({ tid:row.teacher_id, dept:row.dept||'', room:row.room||'', building:row.building||'1' });
    if(State.activeDayKey===key) renderDayPanel(key);
    renderCalendar();
  }
}
...
function _buildWelcomePage(page){
  if(page==='choose')
    return `<div class="wm-logo"><svg viewBox="0 0 44 44" width="48" height="48"><rect width="44" height="44" rx="11" fill="#2C3E50"/><rect x="9" y="18" width="8" height="17" rx="2" fill="#3498DB"/><rect x="18" y="12" width="8" height="23" rx="2" fill="#5DADE2"/><rect x="27" y="8" width="8" height="27" rx="2" fill="#85C1E9"/><circle cx="13" cy="13" r="3.5" fill="#F39C12"/></svg><div><div class="wm-brand">АкадемГрафик</div><div class="wm-sub">Система дежурств</div></div></div><h2 class="wm-title">Добро пожаловать</h2><p class="wm-hint">Выберите режим входа</p><div class="wm-choices"><button class="wm-choice" id="wmChooseAdmin"><span class="wm-choice-icon">🎓</span><span class="wm-choice-label">Завуч</span><span class="wm-choice-sub">Управление расписанием</span></button><button class="wm-choice" id="wmChooseTeacher"><span class="wm-choice-icon">👤</span><span class="wm-choice-label">Преподаватель</span><span class="wm-choice-sub">Просмотр дежурств</span></button></div>`;
  if(page==='login')
    return `<button class="wm-back" id="wmBack">←</button><div style="text-align:center;margin-bottom:1.25rem"><div style="font-size:2.2rem">🔐</div><h2 class="wm-title">Вход для Завуча</h2><p class="wm-hint">Введите логин и пароль</p></div><div><input class="field-input" id="authLogin" placeholder="Логин"><input class="field-input" id="authPassword" type="password" placeholder="Пароль" style="margin-top:.5rem"><div style="margin-top:1rem"><button class="btn-modal-save" id="authSubmit" style="width:100%">Войти</button><span id="authStatus" style="display:block;text-align:center;margin-top:.5rem"></span></div></div>`;
  if(page==='picker'){
    const teachersHtml = State.teachers.map(t=>{
      const color=getColor(teacherIndex(t.id));
      return `<button class="wm-teacher-card" data-tid="${t.id}"><div class="wm-teacher-av" style="background:${color};width:44px;height:44px;font-size:.85rem">${initials(t.name)}</div><div class="wm-teacher-card-name">${t.name}</div><div class="wm-teacher-dept">${t.dept}</div></button>`;
    }).join('');
    return `<button class="wm-back" id="wmBack">←</button><h2 class="wm-title">Выберите преподавателя</h2><input class="field-input" id="wmTeacherSearch" placeholder="🔍 Поиск…" style="margin-bottom:.75rem"><div class="wm-teacher-grid" id="wmTeacherGrid">${teachersHtml}</div>`;
  }
  if(page==='teacher-login'){
    const tid=State._pendingTeacherId;
    const t=teacherById(tid);
    const color=t?getColor(teacherIndex(tid)):'#4A90D9';
    return `<button class="wm-back" id="wmBack">←</button><div style="text-align:center"><div style="width:60px;height:60px;border-radius:50%;background:${color};display:flex;align-items:center;justify-content:center;font-size:1.2rem;font-weight:700;color:#fff;margin:0 auto .75rem">${t?initials(t.name):'?'}</div><h2 class="wm-title">${t?t.name:'Преподаватель'}</h2><p class="wm-hint">Введите пароль</p></div><div><input class="field-input" id="tAuthPassword" type="password" placeholder="Пароль"><div style="margin-top:1rem"><button class="btn-modal-save" id="tAuthSubmit" style="width:100%">Войти</button><span id="tAuthStatus" style="display:block;text-align:center;margin-top:.5rem"></span></div></div>`;
  }
  return '';
}
function _wireWelcomePage(page){
  if(page==='choose'){
    ...
    document.getElementById('wmChooseTeacher')?.addEventListener('click',()=>{ if(State.teachers.length===0){ showToast('Преподаватели ещё не добавлены','error'); return; } showWelcomeModal('picker'); });
  }
  if(page==='login'){
  ...
  if(page==='picker'){
    ...
    const filterRows=()=>{
      ...
      const filtered=State.teachers.filter(t=>t.name.toLowerCase().includes(term));
      grid.innerHTML=filtered.map(t=>{
        const color=getColor(teacherIndex(t.id));
        return `<button class="wm-teacher-card" data-tid="${t.id}"><div class="wm-teacher-av" style="background:${color};width:44px;height:44px;font-size:.85rem">${initials(t.name)}</div><div class="wm-teacher-card-name">${t.name}</div><div class="wm-teacher-dept">${t.dept}</div></button>`;
      }).join('');
      grid.querySelectorAll('.wm-teacher-card').forEach(btn=>btn.addEventListener('click',()=>{
...
function init(){
  ...
  document.getElementById('tPhone')?.addEventListener('input',()=>{ document.getElementById('tPhone').value=formatPhone(document.getElementById('tPhone').value); });
  document.getElementById('addTeacherBtn')?.addEventListener('click',addTeacher);
  renderInlineDeptManager(_inlineDepts);
  document.getElementById('teacherPhone')?.addEventListener('input',()=>{ document.getElementById('teacherPhone').value=formatPhone(document.getElementById('teacherPhone').value); });
  ...
  document.getElementById('hamburger')?.addEventListener('click',()=>{ const nav=document.getElementById('mobileNav'); const open=nav.classList.toggle('open'); document.getElementById('hamburger').classList.toggle('open',open); });
  ...
  document.getElementById('sbStatusClose')?.addEventListener('click',()=>document.getElementById('sbStatusbar')?.classList.add('hidden'));
  document.getElementById('seedDemoBtn')?.addEventListener('click',seedDemoData);
  document.querySelectorAll('.building-tab').forEach(btn=>btn.addEventListener('click',()=>{
    State.activeBuilding = btn.dataset.building;
    _syncBuildingTabs(btn.dataset.building);
    if(State.currentRole==='admin') loadTemplatesList();
    renderCalendar(); renderAccordion();
  }));
