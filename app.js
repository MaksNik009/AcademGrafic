    ...
    assignedEntries.forEach(entry => {
      ...
      html += `<div class="modal-assigned-row">
        <div class="opt-avatar" style="background:${color};width:28px;height:28px;font-size:.7rem;cursor:pointer"
             onclick="openTeacherInfoModal('${entry.tid}')" title="О преподавателе">${initials(t.name)}</div>
...
function addPairEntry(key, pairN) {
  ...
  // Проверка занятости в другом корпусе в этот же день
      ...
      <div class="building-section-table">
        ${byBuilding[b].length === 0
          ...
          : `<table class="teacher-table">
              <thead>
                <tr><th>ФИО</th><th>Кафедра</th><th>Телефон</th>
...
function addPairEntry(key, pairN) {
  ...
  for (const [otherKey, lessons] of Object.entries(State.lessons)) {
    if (otherKey !== key) continue;
    for (const [pn, arr] of Object.entries(lessons)) {
      if (arr.some(e => e.tid === tid && e.building !== State.activeBuilding)) {
        conflictFound = true;
        break;
      }
    }
  ...
  if (conflictFound) {
    showToast(`Преподаватель уже ведёт пару в другом корпусе в этот день!`, 'error');
    return;
  }
  const t = teacherById(tid);
  const depts = Array.isArray(t?.depts) && t.depts.length ? t.depts : [t?.dept].filter(Boolean);
...
async function autoDistribute(useActiveTemplate = true) {
  ...
  // Очищаем дежурства текущего корпуса
  for (const wd of workdays) {
    const entries = getDutyEntries(wd);
